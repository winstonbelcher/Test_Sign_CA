...
/**
 * Represents a finite current carrying wire.
 */
public class FiniteWire extends EMObject implements HasRadius, HasLength, HasCurrent, HasInductance, GeneratesE,
    GeneratesB {
        ...
        if (mNode != null) {
            if ((renderFlags & GEOMETRY_CHANGE) == GEOMETRY_CHANGE) {
                ...
                if (mNode != null) {
                    ...
                    ((FiniteWireNode) mNode).setArrowDirection(current);
                }
                renderFlags ^= GEOMETRY_CHANGE;
    ...
    protected TNode3D makeNode() {
        TNode3D node = new FiniteWireNode(length, radius);
