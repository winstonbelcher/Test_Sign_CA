 ...
 * 
 * $Id: ConductingSphericalShell.java,v 1.1 2008/12/23 18:31:27 jbelcher Exp $ 
 * 
 */
 ...
 */
public class InfiniteConductingPlane extends EMObject implements HasCharge, GeneratesE, GeneratesP {
	...
	public InfiniteConductingPlane() {
		super();
		charge= charge_d= 0.0;
        ...
        radius = 4.;
		height = .1;
		FreeChargePosition = new Vector3d(5.,0.,0.);
		}
	...
	//	double radiusobserver = pos.z; // this is how far the free charge is from the center of the conducting sphere, whose center is at the origin
		double radiusfree = FreeChargePosition.z; // this is how far the observer is from the center of the conducting sphere, whose center is at the origin
		double ro;
		Vector3d yhat = new Vector3d(0.,1.,0.);
		Vector3d r= new Vector3d(0.,0.,0.);
		double radiusobserver = yhat.dot(pos);
		
		if(radiusobserver <= 0.) {  //  this is the case where the observer is below the plane
			
			// here we are calculating the field due to the induced charges 
			// their field in this region is just the negative 
		...
		if( radiusobserver > 0. ) {  
			//  this is the case where the observer is above the plane
			// the e field is the field due to a negative charge below the plane
			
					Vector3d imagePosition= new Vector3d(0.,-8.,0.);
					imagePosition.x = FreeChargePosition.x;
					imagePosition.y = -FreeChargePosition.y;
					imagePosition.z = FreeChargePosition.z;
					
					r.sub(pos, imagePosition);  // the vector from the real charge to the observer
					ro= r.lengthSquared();
					r.normalize();
					r.scale(-1.*this.charge_d / (Teal.fourPiPermVacuum * ro));  // the negative of the electric field due to the real charge
