 ...
 */
public class PointChargeConductingSphere extends EMObject implements HasCharge, GeneratesE, GeneratesP {

    private static final long serialVersionUID = 3257003259120924471L;
    protected double thicknessConductingShell;
	...
	protected double radiusConductingSphere;
	protected double pauliDistance = -1.;
	
	protected double epsilon= 1.0;

	public PointChargeConductingSphere() {
		super();
		charge= charge_d= 0.0;
		...
		fluxMode = 0;
		thicknessConductingShell = 2.;
		radiusConductingSphere=4.;
		selectColor();
		setPickable(true);
		setRotable(false);
		}
	...
	public Vector3d getE(Vector3d pos) {
		...
		double radiusfree = position_d.length(); // this is how far the observer is from the center of the conducting sphere, whose center is at the origin
		double ro;
		Vector3d r= new Vector3d(0.,0.,0.);
		
		if(radiusfree >= radiusConductingSphere) {  //  this is the case where the free charge is outside the shell
			
			if ( radiusobserver < radiusConductingSphere) {  // observer inside the shell
			...
			// of the E field due to the real charge
				r.sub(pos, position_d);  // the vector from the real charge to the observer
				ro= r.lengthSquared();
				r.normalize();
				...
				rimage.scale(radiusConductingSphere*radiusConductingSphere/(radiusfree*radiusfree),position_d);  // this is the location of the image in terms of the location of the real charge
				r.sub(pos, rimage);  // the vector from the image charge to the observer
				ro= r.lengthSquared();
				r.normalize();
				r.scale(-1.*radiusConductingSphere*this.charge_d/(radiusfree*Teal.fourPiPermVacuum * ro));		// the electric field of the image charge
				// now the uniform charge on the surface
				Vector3d r1 =new Vector3d();
				r1.scale(1.,pos);  // vector from the origin to the observer
				ro= r1.lengthSquared();
				r1.normalize();
				r1.scale(this.charge_d*radiusConductingSphere / (radiusfree*Teal.fourPiPermVacuum * ro));
				r.add(r1);
			}
		}
		
		if( radiusfree < radiusConductingSphere && radiusfree >= radiusConductingSphere-thicknessConductingShell) {  
			...
			if ( radiusobserver < radiusConductingSphere) {  // observer inside the shell
				...
				// of the E field due to the real charge
					...
					r2.sub(pos, position_d);  // the vector from the real charge to the observer
					ro= r2.lengthSquared();
					r2.normalize();
			...
			if( radiusfree < radiusConductingSphere-thicknessConductingShell) {  
				//  this is the case where the free charge is in the interior of the shell, not in the conducting part
				
				if ( radiusobserver <= radiusConductingSphere-thicknessConductingShell) {  // observer inside the conductor
					...
					Vector3d rimage = new Vector3d();
					double innerradius = radiusConductingSphere - thicknessConductingShell;
					rimage.scale(innerradius*innerradius/(radiusfree*radiusfree),position_d);  // this is the location of the image in terms of the location of the real charge
					r.sub(pos, rimage);  // the vector from the image charge to the observer
					ro= r.lengthSquared();
					r.normalize();
				...
				if ( radiusobserver <= radiusConductingSphere && radiusobserver > radiusConductingSphere-thicknessConductingShell) {  // observer inside the conductor
				...
				if ( radiusobserver > radiusConductingSphere) {  // observer outside the shell
			...
			// now we add the electric field of the point charge itself
			Vector3d r3= new Vector3d();
			r3.sub(pos, position_d);
			double ro3= r3.lengthSquared();
			r3.normalize();
			r3.scale(this.charge_d / (Teal.fourPiPermVacuum * ro3));
			r.add(r3);
	return r;
	}
