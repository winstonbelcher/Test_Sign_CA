 ...
 */
public class CurrentSlab extends EMObject implements HasHeight, HasLength, HasWidth, HasCurrent, HasInductance, GeneratesE,
    GeneratesB {
    ...
    public CurrentSlab(double height, double width, double length) {
        super();
        isMoveable = false;
        setCurrent(Teal.InfiniteWireDefaultCurrent);
        setMass(Teal.InfiniteWireMass);
    ...
    public Vector3d getB(Vector3d pos) {
        ...
        if (pos.y >= length/2.) B = new Vector3d(-1.5,0.,0.);
        if (pos.y <= -length/2.) B = new Vector3d(1.5,0.,0.);
        if (Math.abs(pos.y)< length/2.) {
        	double ratio = -1.5*(2.*pos.y)/length;
