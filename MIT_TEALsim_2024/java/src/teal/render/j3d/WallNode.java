 ...
 * 
 * $Id: WallNode.java,v 1.11 2007/07/16 22:04:55 pbailey Exp $ 
 * 
 */
...
public class WallNode extends Node3D implements HasColor /*MultiShapeNode*/ {
	...
	public WallNode() {
		...
		fillShape.setAppearance(Node3D.makeAppearance(new Color3f(Color.GRAY),0.5f,transWall,false));
	...
	public void setColor(Color3f c) {	
		Appearance app = Node3D.makeAppearance(c,0.5f,transWall,false);
		fillShape.setAppearance(app);
	
	}
	
	public float getTrans() {
		TransparencyAttributes transapp = fillShape.getAppearance().getTransparencyAttributes();
		float trans;
		trans = transapp.getTransparency();
		return trans;
	}
