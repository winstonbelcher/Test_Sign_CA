 ...
 * 
 * $Id: FieldLine.java,v 1.97 2008/07/15 10:03:30 jbelcher Exp $ 
 * 
 */
...
import teal.render.j3d.FieldLineNode;
...
public class FieldLine extends SpatialField {
    ...
    /** the displayState of the line independant of the isDrawn variable */
    ...
    protected float[] nData;
    protected float[] pData;
    ...
    protected Vector markerPositions = null;
    ...
    /////////////////////////
    protected float[] nColors;
    protected float[] pColors;
    ...
    public void setPickRadius(double r) {
        ...
        if (mNode != null) {
            ...
            ((FieldLineNode) mNode).setPickRadius(pickRadius);
            ((FieldLineNode) mNode).setPickVisible(showPick);
    ...
    public void setShowPick(boolean b) {
        ...
        showPick = b;
        if (mNode != null) {
            ((FieldLineNode) mNode).setPickVisible(b);
        }
        if(theEngine != null)
            theEngine.requestRefresh();
    }
    ...
    protected void setMarkersVisible(boolean b) {
        if (mNode != null) {
            for (int i = 0; i < numMarkers; i++) {
                ((FieldLineNode) mNode).setMarkerVisible(i, b);
            }
    ...
    public void render() {
        ...
        if (symmetryChanged) {
            ...
            ((FieldLineNode) mNode).setSymmetry(numClones, symAxis);
        ...
        if ((renderFlags & GEOMETRY_CHANGE) == GEOMETRY_CHANGE) {
            setLineGeometry((FieldLineNode) mNode);
            updateMarkers();
            renderFlags ^= GEOMETRY_CHANGE;
    ...
    protected void checkMarkers() {
        if (mNode != null) {
            ((FieldLineNode) mNode).checkMarkers(numMarkers);
        }
    }
    ...
    protected void updateMarkers() {
        if (markerPositions != null) {
            Iterator it = markerPositions.iterator();
            int i = 0;
            while (it.hasNext() && i < numMarkers) {
                Vector3d pos = (Vector3d) it.next();
                ((FieldLineNode) mNode).setMarkerValues(i, pos, field.get(pos));
                i++;
            }
        }
    ...
    protected void setLineGeometry(FieldLineNode node) {
        if (colorMode == COLOR_VERTEX || colorMode == COLOR_VERTEX_FLAT) {
            ...
            if (field != null) {
                ...
                while (k < kMax && loop) {
                    ...
                    if (colorMode == COLOR_VERTEX_FLAT) {
                        ...
                        //colors[coloroff++] = (float)(k%2);
                        //colors[coloroff++] = (float)(k%2);
                        colors[coloroff++] = vred * (float) Math.log(2. * Math.log(x + 1.) + 1.);
                        colors[coloroff++] = vgreen * (float) Math.log(2. * Math.log(x + 1.) + 1.);
                        colors[coloroff++] = vblue * (float) Math.log(2. * Math.log(x + 1.) + 1.);
                        ////////////////////////////////////
                    }
                    // Closed line check
