/* $Id: NewtonsCradle.java,v 1.5 2007/11/08 18:18:15 pbailey Exp $ */
/**
...
public class NewtonsCradle extends SimWorld {
	...
	PhysicalObject m1 = null, m2 = null, m3 = null, m4 = null, m5 = null;
	...
	public NewtonsCradle() {
		...
		Color3f sphereColor = new Color3f(Color.GRAY);
    ...
    /*
       ...
       */ 
		...
		((EMEngine)theEngine).setDamping(0.);
		((EMEngine)theEngine).setGravity(new Vector3d(0., -gravity, 0.));
	...
	public void propertyChange(PropertyChangeEvent pce) {
		...
		if(source == m1) {
			...
			if(state != TEngineControl.RUNNING)
    			((EMEngine)theEngine).requestReorder(m1);
		} else if(source == m2) {
			int state = mSEC.getSimState();
			if(state != TEngineControl.RUNNING)
				((EMEngine)theEngine).requestReorder(m2);
		} else if(source == m3) {
			int state = mSEC.getSimState();
			if(state != TEngineControl.RUNNING)
				((EMEngine)theEngine).requestReorder(m3);
		} else if(source == m4) {
			int state = mSEC.getSimState();
			if(state != TEngineControl.RUNNING)
				((EMEngine)theEngine).requestReorder(m4);
		} else if(source == m5) {
			int state = mSEC.getSimState();
			if(state != TEngineControl.RUNNING)
				((EMEngine)theEngine).requestReorder(m5);
