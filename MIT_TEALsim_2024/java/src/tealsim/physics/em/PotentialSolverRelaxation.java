/* $Id: ExploringPotential.java,v 1.9 2008/08/18 14:09:05 jbelcher Exp $ */
/**
 * @author John Belcher 
 * Revision: 1.0 $
 */
...
public class PotentialSolverRelaxation extends SimEM {
    ...
    public PotentialSolverRelaxation() {
        super();

        TDebug.setGlobalLevel(0);

        title = "Potential Relaxation Solver";
        ...
        // We create a two D array of potential levels.
		// Wall constructor.  	
        
        wall = new Wall[nwall][nwall];
        PotentialSet = new boolean[nwall][nwall];
   ...
   //     addElement(vizPanel);
    ...
    public void moveObserver(){
      ...
      //  mViewer.setLookAt(new Point3d(postot.x,postot.y,postot.z), 
    	...
    	if(!PotentialSet[idxX][idxY]){
    		moveWall(idxX,idxY);
    		PotentialSet[idxX][idxY] = true;
    		numMoves++;
    	}
  ...
  //  	optionsGroup.clearSelection();
    	messages.setText("");
    	clearText = false;
    	// reset potential levels
    	 for (int i = 0; i < nwall; i++) 
         {
         	for (int j = 0; j < nwall; j++) 
         	{         		
         		wall[i][j].setPosition(getPosition(i,j));
         		PotentialSet[i][j] = false;
         	} 	
         }
