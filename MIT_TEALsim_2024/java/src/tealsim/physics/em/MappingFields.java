/* $Id: MappingFields.java,v 1.9 2008/02/11 19:58:09 pbailey Exp $ */
...
public class MappingFields extends SimEM implements ImageStatusListener, SelectListener, DocumentListener {
    ...
    public MappingFields() {
        ...
        // *****************************************************************
        mViewer.setNavigationMode(TViewer.ORBIT | TViewer.VP_TRANSLATE | TViewer.VP_ZOOM);
        mViewer.setRefreshOnDrag(true);
        mViewer.setShowGizmos(false);

        theEngine.setBoundingArea(new BoundingSphere(new Point3d(), max_scale));
        ...
        //theEngine.addSimElement(uf);
        GenericEField efield = new GenericEField(uf);
        theEngine.addSimElement(efield);
 ...
 //       addElement(centerx);   these four lines commented out jwb 9/2010
 //       addElement(centery);
 ...
 //       addElement(label_centery); 
        ...
        vectorfield.setText("Vector Field (input fns or use Examples)");
   ...
   //     addElement(zoombox);    commented out jwb 9/2010
        ...
        params.setText("Save Image Generated");
   //     params.add(slider1);
        saveButton = new JButton(new TealAction("Save Image to JPG", "Save Image to JPG", this));
        saveButton.setFont(saveButton.getFont().deriveFont(Font.BOLD));
        saveButton.setBounds(40, 570, 195, 24);
        ...
        mSEC.init();
        mSEC.rebuildPanel(0);  // this commands hides the run controls at the bottom of the window
        reset();
        //layoutNormal();
    }
    ...
    public void actionPerformed(ActionEvent e) {
         ...
         */
            ...
            propertyChange(new PropertyChangeEvent(wall, "position", new Vector3d(wall.getPosition()), new Vector3d(
                position)));
        } else if (e.getActionCommand().compareToIgnoreCase("Save Image to JPG") == 0) {
            saveDLICImage();
        } else {
    ...
    public void autoTranslateZoom() {
        ...
        double scale = ((Double) scale_slider.getValue()).doubleValue();
        position.scale(0.05);
        mViewer.setLookAt(new Point3d(position.x, position.y, scale / 7.5), new Point3d(position.x, position.y, 0.),
            new Vector3d(0., 1., 0.));
    }
    ...
    protected void saveDLICImage() {
        if (mDLIC.isImageGenerated()) {
            ...
            if (img != null) {
                ...
                int status = fc.showSaveDialog(mFramework.getTheWindow());
    ...
    public void resetCamera() {
        ...
        mViewer.setLookAt(new Point3d(initial_position.x, initial_position.y, initial_scale / 7.5), new Point3d(
