 /* $Id: Circuits.java,v 1.3 2007/08/14 20:53:24 pbailey Exp $ */
...
//		 Building the world.
		...
		engine.setBoundingArea(bs);
		engine.setDamping(0.);
		engine.setGravity(new Vector3d(0., 0., 0.));
		...
		// Circuit assembly.
		((EMEngine)theEngine).setDeltaTime(stepSize);
		Circuit.mu0 = mu0;
	...
	public void propertyChange(PropertyChangeEvent pce) {
		if( pce.getSource() == theEngine && pce.getPropertyName().equalsIgnoreCase("simState")) {
			if( mSEC.getSimState() == EngineControl.PAUSED) {
				System.out.println("Paused - Time: " + ((EMEngine)theEngine).getTime());
				circuit.inform();
			}
		}
