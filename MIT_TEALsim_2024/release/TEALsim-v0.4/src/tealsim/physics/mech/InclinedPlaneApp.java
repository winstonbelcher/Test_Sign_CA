/* $Id: InclinedPlaneApp.java,v 1.8 2007/09/04 21:27:05 pbailey Exp $ */
...
public class InclinedPlaneApp extends SimWorld {
	...
	public InclinedPlaneApp() {
		...
		emModel.setGravity( new Vector3d( 0., -gravity, 0.) );
		emModel.setShowTime(false);
		emModel.setDeltaTime(0.01);
		setEngine(emModel);
		...
		slope.normalize();
		slope.scale(((SpringConstraint)slidingbox.getConstraintAtIndex(0)).getRestLength() + slidingbox.getMass()*emModel.getGravity().length()*Math.sin(iAngle));
...
//		 HACK:  I can't call reset here since the OrientedShape3D of the SpatialTextLabels throws an exception
        ...
        ((ViewerJ3D)mViewer).setAlternateAppearance(null);
	}
    ...
    public void resetCamera() {
    	...
    	mViewer.setLookAt(new Point3d(0,0.05,0.2), new Point3d(0,0.05,0), new Vector3d(0,1,0));
    }
	...
	public void setAngle(double angle) {
		...
		slope.normalize();
		slope.scale(((SpringConstraint)slidingbox.getConstraintAtIndex(0)).getRestLength() + (slidingbox.getMass()*emModel.getGravity().length()*Math.sin(iAngle))/((SpringConstraint)slidingbox.getConstraintAtIndex(0)).getCoefficient());
		Vector3d angleeqpos = new Vector3d(((SpringConstraint)slidingbox.getConstraintAtIndex(0)).getPoint());
		angleeqpos.add(slope);
