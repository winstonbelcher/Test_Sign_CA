/* $Id: ConductingSphericalShellShielding.java,v 1.1 2008/12/23 19:51:17 jbelcher Exp $ */
...
public class ConductingSphericalShellShieldingHudson extends SimEM {
    ...
    PointCharge pcharge;
    ConductingSphericalShell conductingShell, conductingShell1;
    Rendered ring;
    TShapeNode RingNode = (TShapeNode) new ShapeNode();
    TShapeNode RingNode1 = (TShapeNode) new ShapeNode();
    PhysicalObject cylinder;
    ...
    JRadioButton rad3;
    JRadioButton rad4;
    Rendered[] InducedChargeSpheresInner;
    ShapeNode[] ShapeNodeInducedChargesInner;
    Rendered[] InducedChargeSpheresOuter;
    ShapeNode[] ShapeNodeInducedChargesOuter;
    double signFreeCharge;
    double anglePC = 0.;

    public ConductingSphericalShellShieldingHudson() {
	    ...
	    //  create the spheres showing the induced charges and their shapenodes
         InducedChargeSpheresInner = new Rendered[4*Ninduced];
         ShapeNodeInducedChargesInner = new ShapeNode[4*Ninduced];
         InducedChargeSpheresOuter = new Rendered[4*Ninduced];
         ShapeNodeInducedChargesOuter = new ShapeNode[4*Ninduced];
        for (int j = 0; j < 4*Ninduced ; j++) {
     		InducedChargeSpheresInner[j] = new Rendered();
     		ShapeNodeInducedChargesInner[j]=new ShapeNode();
     		ShapeNodeInducedChargesInner[j].setGeometry(Sphere.makeGeometry(32, radiusInduced));
     		InducedChargeSpheresInner[j].setNode3D(ShapeNodeInducedChargesInner[j]);
     		...
     		InducedChargeSpheresOuter[j] = new Rendered();
     		ShapeNodeInducedChargesOuter[j]=new ShapeNode();
     		ShapeNodeInducedChargesOuter[j].setGeometry(Sphere.makeGeometry(32, radiusInduced));
     		InducedChargeSpheresOuter[j].setNode3D(ShapeNodeInducedChargesOuter[j]);
        ...
        rad4 = new JRadioButton("Show No Field");
	...
	//	options.add(rad4);
        ...
        conductingShell.setRadius(radius);
        conductingShell.setThickness(thickness);
        RingNode.setGeometry(Pipe.makeGeometry(50, radius-thickness/2, thickness, height));
        RingNode.setPickable(false);
        RingNode.setColor(new Color3f(Color.ORANGE));
        RingNode.setTransparency(0.0f);
        conductingShell.setNode3D(RingNode);
        conductingShell.setColor(new Color3f(Color.gray));
    ...
    //    conductingShell.addPropertyChangeListener("value",this);
        ...
        // Creation and initialization of another conducting shell
        conductingShell1 = new ConductingSphericalShell();
        conductingShell1.setID(" ConductingSphericalShell1 ");
        conductingShell1.setPosition(new Vector3d(0, 0, 0));
        conductingShell1.setFreeChargePosition(new Vector3d(8, 0, 0));
        conductingShell1.setRadius(radiusPointCharge);
        conductingShell1.setCharge(0);
        conductingShell1.setMass(1.);
        conductingShell1.setSelectable(false);
        conductingShell1.setPickable(false);
        conductingShell1.setColliding(false);
        conductingShell1.setRadius(radius);
        conductingShell1.setThickness(thickness);
        double radius1 = radius-thickness;
        RingNode1.setGeometry(Pipe.makeGeometry(50, radius1/2., radius1, height));
        RingNode1.setPickable(false);
        RingNode1.setColor(new Color3f(Color.ORANGE));
        RingNode1.setTransparency(0.0f);
        conductingShell1.setNode3D(RingNode1);
        conductingShell1.setColor(new Color3f(Color.black));
        conductingShell1.setDirection(new Vector3d(0.,0.,1.));
    //    conductingShell.addPropertyChangeListener("value",this);
        addElement(conductingShell1);
    	 ...
    	 if (radiusfree < radius-thickness) {
          ...
          // first place charges on outer surface, uniformly with the same sign as the free charge
    	     for (int i = 0; i<2*Ninduced; i++){
    	    	 ...
    	    	 InducedChargeSpheresOuter[i].setDrawn(false);
    	    	 ...
    	    	 InducedChargeSpheresOuter[i+2*Ninduced].setDrawn(false);
    	     }
    	 	 ...
    	 	 if (radiusfree >= radius) {
        	    ...
        	    // System.out.println(" ratio "+ ratio +" coszero " + coszero + " qincmax "+ qincmax);
        	     for (int i = 0; i<Ninduced; i++){
        	    	 ...
        	    	 InducedChargeSpheresOuter[i+Ninduced].setDrawn(false);
        	     ...
        	     for (int i = 0; i<Ninduced; i++){
        	    	 ...
        	    	 InducedChargeSpheresOuter[i+3*Ninduced].setDrawn(false);
