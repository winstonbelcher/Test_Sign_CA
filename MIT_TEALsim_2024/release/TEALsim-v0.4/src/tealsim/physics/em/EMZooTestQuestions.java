 ...
 */
public class EMZooTestQuestions extends SimEM implements SelectListener {
    ...
    public EMZooTestQuestions() {

        super();
        //setID("Zoo");
        title = "Electrostatic Zoo Test Questions";
    ...
    void addActions() {
        ...
        but7 = new JButton(new TealAction("Make Charges Invisible", "invisible", this));
        but7.setBounds(250, 690, 195, 24);
        //addElement(but4);
        
        but8 = new JButton(new TealAction("Make Charges Visible", "visible", this));
        but8.setBounds(250, 690, 195, 24);
        //addElement(but4);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().compareToIgnoreCase("EM Zoo") == 0) {
            ...
            clearAllCharges();
        } else if (e.getActionCommand().compareToIgnoreCase("invisible") == 0) {
            MakeChargesInvisible();
        } else if (e.getActionCommand().compareToIgnoreCase("visible") == 0) {
            MakeChargesVisible();
...
*/
    ...
    private void MakeChargesInvisible() {
        int simstate = mSEC.getSimState();
        mSEC.stop();
        ...
        while (it.hasNext()) {
            ...
            if (em instanceof PointCharge) {
               // removeFLinesFromObject((PhysicalObject) em);
               // removeElement((PointCharge) em);
            	PointCharge nebie = (PointCharge) em;
            	nebie.setDrawn(false);
            }
        }
        //mSEC.step();
        if (simstate == TEngineControl.RUNNING)
    ...
    private void MakeChargesVisible() {
        int simstate = mSEC.getSimState();
        mSEC.stop();
