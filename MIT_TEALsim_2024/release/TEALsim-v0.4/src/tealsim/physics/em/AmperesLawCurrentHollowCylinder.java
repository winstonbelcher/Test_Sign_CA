...
import teal.physics.em.HollowCurrentShell;
...
public class AmperesLawCurrentHollowCylinder extends SimEM  {
	/**
	 * 
	 */
    ...
    boolean rectangleVisible = false;
    ...
    public AmperesLawCurrentHollowCylinder() {
        super();
        // Hide run controls       
        mSEC.rebuildPanel(0);
        // Set debug level
 ...
 //       addWall(new Vector3d(0., 0., -wheight/2.), new Vector3d(0., length, 0.), new Vector3d( width+length, 0., 0.));
        ...
        AmpereanCircle.setPosition(new Vector3d(0.,0.,0.));
        ...
        ShapeNodeARectangle = new AmpereanRectangleNode3D(radiusAcircle,3.,.02);
        ...
        posSlider_y.setText("Radius Circle ");
        posSlider_y.setMinimum(.0);
        posSlider_y.setMaximum(6.0);
        ...
        PositionOrientationControlGroup.setText("Radius of Amperean Loop");
 ...
 //       addElement(LICvisualizationControl);
        ...
        addElement(CreateHollowCurrentShell(radius,radius,radius,radius));
 ...
 // Set initial state
    ...
    void addActions() {
        ...
        TealAction tb = new TealAction("Ampere's Law", this);
        addAction("Help", tb);
  ...
  /** Method to handle the action performed events.  */  
    		...
    		AmpereanCircle.setNode3D(ShapeNodeACircle);
  	        ...
  	        AmpereanLoop = ShapeNodeACircle.getTransform3D();
  	    	 ...
  	    	 for (int j=1; j<=8; j++) {
  	             ...
  	             newPosition.x = WorldLocationTangentVectorJ.x;
  	             newPosition.y = WorldLocationTangentVectorJ.y;
  	             newPosition.z = 0.;
    ...
    public void propertyChange(PropertyChangeEvent pce) {
        ...
        if (source == posSlider_x) {
            ...
            AmpereanRectangle.setPosition(posARectangle);
          //  posACircle.y = posY;
            if(posY > .1) radiusAcircle = posY; else radiusAcircle = .1;
    ...
    /** Method to create a shell of current*/
    private  HollowCurrentShell CreateHollowCurrentShell(double current, double height, double width, double length) {
        HollowCurrentShell newHollowCurrentShell = new HollowCurrentShell();
        newHollowCurrentShell.setCurrent(current);
        newHollowCurrentShell.setDirection(new Vector3d(0.,0.,1.));
        newHollowCurrentShell.setMass(1.);
        newHollowCurrentShell.setHeight(height);
        newHollowCurrentShell.setWidth(width);
        newHollowCurrentShell.setRadius(radius);
        newHollowCurrentShell.setSelectable(true);
        newHollowCurrentShell.setPickable(true);
        newHollowCurrentShell.setColliding(true);

        newHollowCurrentShell.addPropertyChangeListener(this);

        
        Vector3d testPos = new Vector3d(0.,0.,0.);
        newHollowCurrentShell.setPosition(testPos);
        return newHollowCurrentShell;
    }
    ...
    private void calculateCurrentThru(){
    	...
    	// Interate over each point line current in the scene and see if it is inside the rectangle, if so increment chargeInCylinder
    	
	    double currentThruRectangle = 0.;
	    
	    if (Math.abs(posY) <= radius) currentThruRectangle=1.;
	    ...
	    double currentThruCircle = 0.;
	    if (Math.abs(posY) <= radius) currentThruCircle=1.;
