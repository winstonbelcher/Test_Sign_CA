 ...
 * 
 * $Id: FluxFieldLine.java,v 1.68 2008/01/06 12:26:25 jbelcher Exp $ 
 * 
 */
...
public class FluxFieldLine extends FieldLine implements HasReference {
    ...
    public void nextSpatial() {
        if (isDrawn) {
            ...
            if (newPos == null) {
                ...
                setPosition(newPos);
                TDebug.println(1,"nextSpatial Root" + newPos.toString());
                showNode = true;
                renderFlags |= VISIBILITY_CHANGE;
                super.nextSpatial();
    ...
    private double[] TEALRoots(double xs, double xf, int intervals) {
    	...
    	for (int i = 0; i < intervals; i++) {
            ...
            TDebug.println(1,"FluxFieldLine Search:  i " +i+ " x1 " + x1 + " f1 " +f1 + " flux " + fluxValue + " VectorPos " + mapTo3D(x1).toString()) ;
    		if (f1*f2 < 0.) {
